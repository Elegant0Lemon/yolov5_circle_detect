...
def process_wandb_config_ddp_mode(opt):
    ...
    if isinstance(data_dict['train'], str) and data_dict['train'].startswith(WANDB_ARTIFACT_PREFIX):
        ...
        train_path = Path(train_dir) / 'data/image/'
    ...
    if isinstance(data_dict['val'], str) and data_dict['val'].startswith(WANDB_ARTIFACT_PREFIX):
        ...
        val_path = Path(val_dir) / 'data/image/'
...
class WandbLogger():
    ...
    def setup_training(self, opt):
        ...
        if self.train_artifact_path is not None:
            train_path = Path(self.train_artifact_path) / 'data/image/'
            data_dict['train'] = str(train_path)
        if self.val_artifact_path is not None:
            val_path = Path(self.val_artifact_path) / 'data/image/'
            data_dict['val'] = str(val_path)
    ...
    def create_dataset_table(self, dataset: LoadImagesAndLabels, class_to_id: Dict[int, str], name: str = 'dataset'):
        ...
        for img_file in img_files:
            if Path(img_file).is_dir():
                artifact.add_dir(img_file, name='data/image')
                labels_path = 'labels'.join(dataset.path.rsplit('image', 1))
                artifact.add_dir(labels_path, name='data/labels')
            else:
                artifact.add_file(img_file, name='data/image/' + Path(img_file).name)
