...
class ClearmlLogger:
    ...
    def __init__(self, opt, hyp):
        ...
        # Keep tracked of amount of logged image to enforce a limit
        self.current_epoch_logged_images = set()
        # Maximum number of image to log to clearML per epoch
        self.max_imgs_to_log_per_epoch = 16
        # Get the interval of epochs when bounding box image should be logged
    ...
    def log_debug_samples(self, files, title='Debug Samples'):
        """
        Log files (image) as debug samples in the ClearML task.

        arguments:
        files (List(PosixPath)) a list of file paths in PosixPath format
        title (str) A title that groups together image with the same values
        """
        for f in files:
            if f.exists():
