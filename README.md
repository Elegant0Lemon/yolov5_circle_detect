...
```bash
bash data/scripts/get_coco.sh --val --segments  # download COCO val segments split (780MB, 5000 image)
...
```bash
python segment/predict.py --weights yolov5m-seg.pt --data data/image/bus.jpg
```
```python
...
```bash
bash data/scripts/get_imagenet.sh --val  # download ImageNet val split (6.3G, 50000 image)
...
```bash
python classify/predict.py --weights yolov5s-cls.pt --data data/image/bus.jpg
```
```python
...
=======
# yolov5_circle_detect
这是一个用于识别基于纸片扩散法（K-B法）的药敏试验抑菌圈的模型。
选用的是yolov5m模型
>>>>>>> e7bf00b1f6b71dc6295444ec25b2c2420d7e2e18
